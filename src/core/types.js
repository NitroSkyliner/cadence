...
let counter = 0
export function createPost({ text, platforms, scheduledAt, status = STATUS.SCHEDULED, repeat = REPEAT.NONE, media = [], thread = [], variants = {}, first_comment = '' }) {
  return {
    id: `post_${Date.now()}_${counter++}`,
    text, platforms, scheduledAt,
    status, results: {}, metrics: {}, repeat, media, thread, variants, first_comment, createdAt: Date.now(),
  }
}
